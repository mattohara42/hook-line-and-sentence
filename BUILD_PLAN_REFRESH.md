...
### R7 — Gear in the new style (code half done 2026-09-02; **all 12 hats landed the same day; 5 rods remain, and the pipeline now takes a delivery unchanged**)
...
| R7 | 21 paintings, one per gear item per pose, each an **edit of that pose's own painting** (`ART.md` has all 21 prompts written out). **16 landed, 5 to go** — the hat column is complete and four non-gate rods are in kids' hands. A hat is one `tools/cut-gear.py` run and one `rig.gearArt` line; a rod is a `tools/cut-angler.py` run instead, because the shaft under the hand is where the two paintings agree |
