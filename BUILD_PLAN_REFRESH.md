...
### R7 — Gear in the new style (code half done 2026-09-02; **all 12 hats landed the same day; 8 rods remain, and the pipeline for them is proven**)
...
| R7 | 21 paintings, one per gear item per pose, each an **edit of that pose's own painting** (`ART.md` has all 21 prompts written out). **13 landed, 8 to go** — the hat column is complete and `rod-stick-stream` is the first non-gate rod. A hat is one `tools/cut-gear.py` run and one `rig.gearArt` line; a rod is a `tools/cut-angler.py` run instead, because the shaft under the hand is where the two paintings agree |
