...
### R7 — Gear in the new style (code half done 2026-09-02; **all 12 hats landed the same day; 6 rods remain, and the pipeline is proven at all three poses**)
...
- **Still open, and not part of the done-when:** the nine non-gate rods. Each
  ...
  the hand is exactly where the two paintings agree. Writing the nine prompts out
  turned up one thing worth carrying: **three of that corridor's numbers are per
  rod, not per pose** (the reel circle, the shaft half-width, and the Stream's
  left bound), because each was measured off the gate rod that pose happens to
  hold. It is flagged rather than solved, with the cheap shape sketched, in
  `ART.md` → *The rods* — the first delivered rod is what settles it.
...
| R7 | 21 paintings, one per gear item per pose, each an **edit of that pose's own painting** (`ART.md` has all 21 prompts written out). **15 landed, 6 to go** — the hat column is complete and three non-gate rods are in kids' hands. A hat is one `tools/cut-gear.py` run and one `rig.gearArt` line; a rod is a `tools/cut-angler.py` run instead, because the shaft under the hand is where the two paintings agree |
